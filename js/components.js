...
function departmentCard(key) { const d = SITE_CONFIG.departments[key]; return `<article class="card department-card department-accent reveal" style="--department-accent:${d.accentColor};--department-contrast:${contrastColor(d.accentColor)}"><img data-department-logo="${key}" width="112" height="112" loading="lazy" alt="${d.name} logo placeholder"><p class="eyebrow">${d.abbreviation}</p><h3>${d.name}</h3><p>${d.description}</p><div class="department-card__actions">${internalButton(d.page, `View ${d.abbreviation}`, "button button--department")}${applicationButton(key, `Apply to ${d.abbreviation}`)}</div></article>`; }
