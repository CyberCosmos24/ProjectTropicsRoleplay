...
function contrastColor(hex) { const channels = hex.slice(1).match(/.{2}/g).map(value => parseInt(value, 16) / 255).map(value => value <= .04045 ? value / 12.92 : ((value + .055) / 1.055) ** 2.4); const luminance = .2126 * channels[0] + .7152 * channels[1] + .0722 * channels[2]; return luminance > .179 ? "#000000" : "#FFFFFF"; }
...
function hero({ key, eyebrow = "Project Tropics Roleplay", title, text, logo, siteLogo = false, actions = "" }) {
  const heroLogo = siteLogo
    ? `<img class="hero__logo" data-site-logo width="180" height="180" alt="Project Tropics Roleplay logo">`
    : logo ? `<img class="hero__logo" data-department-logo="${logo}" width="180" height="180" alt="${SITE_CONFIG.departments[logo].name} logo">` : "";
  return `<section class="hero" data-background="${key}"><div class="hero__overlay"></div><div class="container hero__content reveal">${heroLogo}<p class="eyebrow">${eyebrow}</p><h1>${title}</h1><p class="hero__lead">${text}</p><div class="button-row">${actions}</div></div></section>`;
}
...
function departmentCard(key) { const d = SITE_CONFIG.departments[key]; return `<article class="card department-card department-accent reveal" style="--department-accent:${d.accentColor};--department-contrast:${contrastColor(d.accentColor)}"><img data-department-logo="${key}" width="112" height="112" loading="lazy" alt="${d.name} logo"><p class="eyebrow">${d.abbreviation}</p><h3>${d.name}</h3><p>${d.description}</p><div class="department-card__actions">${internalButton(d.page, `View ${d.abbreviation}`, "button button--department")}${applicationButton(key, `Apply to ${d.abbreviation}`)}</div></article>`; }
...
function renderNavbar() {
  ...
  const externalLink = (href, label) => `<a href="${escapeHTML(href)}" target="_blank" rel="noopener noreferrer">${label}</a>`;
